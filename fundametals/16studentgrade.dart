//16. Write a program user enter the 5 subjects mark. You have to make a total and find the percentage. percentage > 75 you have to print “Distinction” percentage > 60 and percentage <= 75 you have to print “First class” percentage >50 and percentage <= 60 you have to print “Second class” percentage > 35 and percentage <= 50 you have to print “Pass class” Otherwise print “Fail”

import 'dart:io';

void main(){


  print("Enter sub1 marks ");
  var sub1 = int.parse(stdin.readLineSync()!);
  print("Enter sub2 marks ");
  var sub2 = int.parse(stdin.readLineSync()!);
  print("Enter sub3 marks ");
  var sub3 = int.parse(stdin.readLineSync()!);
  print("Enter sub4 marks ");
  var sub4 = int.parse(stdin.readLineSync()!);
  print("Enter sub5 marks ");
  var sub5 = int.parse(stdin.readLineSync()!);

  var sub_total = sub1+sub2+sub3+sub4+sub5;

  print("total obtained marks is =  $sub_total");

  var percentage = (sub_total*100)/500;

  print("Student percetage is $percentage");

  if(percentage>75){
    print("“Distinction");
  }
  else if(percentage>60 && percentage<=75){
    print("Student got First Class");
  }
  else if(percentage>50 && percentage<=60){

    print("Student got Second Class");
  }

  else if(percentage>35 && percentage<=50){

    print("Student got Pass Class");
  }
  else{
    
    print("You are Fail");
  }
}